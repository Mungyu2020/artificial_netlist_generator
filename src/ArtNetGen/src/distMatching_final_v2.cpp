...
using namespace odb;
/* VERSION 1
1. Determine bbox m of each net from Bbox dist.
2. get a sample k from Fanout dist. -> Determine # of candidate sink
3. Phase1: determine outer edge of length m
...
Netlist::distMatching() {
    ...
    // sample_k = # of candidate sink
    // sample_s = edge length 
    // 2. get a sample k from Fanout dist. -> Determine # of candidate sink
    int sample_k = foDist_.sample();
    // 2. get k samples S from edge dist.
    vector<int> sample_S;
    for(int i = 0; i < sample_k; ++i) {
        int s = edgeDist_.sample();
        sample_S.push_back(s);
    }
    auto max_it = std::max_element(sample_S.begin(), sample_S.end());
    int bboxOfNet = *max_it;
    sample_S.erase(max_it);
