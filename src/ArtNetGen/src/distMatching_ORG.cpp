    ...
    while ( tarEdgeCnt > curEdgeCnt ) {
        ...
        for (Bin* srcBin : srcBins) {
            ...
            for(Bin* sinkBin : sinkBins) {
                auto t2 = Clock::now();
                //Gain localMaxG = getMaxGain_v3(srcBin, sinkBin);
                //Gain localMaxG = getMaxGain_v3_cuda(srcBin, sinkBin);
                //Gain localMaxG = getMaxGain_v2(srcBin, sinkBin);
